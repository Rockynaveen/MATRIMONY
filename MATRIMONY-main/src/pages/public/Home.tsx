...
export const Home: React.FC = () => {
  const { profiles, isAuthenticated } = useApp();
        ...
        <div className="mx-auto max-w-7xl px-4 sm:px-6 lg:px-8 relative z-10">
          <ScrollReveal direction="up">
            ...
            <div className="grid grid-cols-1 md:grid-cols-3 gap-8 items-stretch max-w-6xl mx-auto">
              {MOCK_MEMBERSHIP_PLANS.slice(1).map(plan => {
                    ...
                    <div className={isPopular || isVIP ? 'pt-3' : ''}>
                      ...
                      <div className="flex items-start justify-between pb-5 border-b border-current/15">
                        <div>
                          <div className="flex items-center gap-2">
                            <h3 className={`font-serif text-2xl font-extrabold ${isPopular || isVIP ? 'text-white' : 'text-stone-900'}`}>
                              {plan.name}
                            </h3>
                          </div>
                          <p className={`text-xs mt-1 font-semibold ${isPopular || isVIP ? 'text-stone-200' : 'text-stone-500'}`}>
                            Validity: <span className="font-extrabold text-amber-300">{plan.period}</span>
                          </p>
                        </div>

                        {plan.originalPrice && (
                          <span className={`text-[10px] font-extrabold px-3 py-1 rounded-full uppercase tracking-wider ${isPopular ? 'bg-amber-400 text-stone-950 font-bold' : isVIP ? 'bg-amber-400/20 text-amber-300 border border-amber-400/40' : 'bg-emerald-100 text-emerald-800'
                            }`}>
                      ...
                      <div className="my-6">
                        <div className="flex items-baseline gap-2">
                          ...
                          {plan.originalPrice && (
                            <span className={`text-sm font-semibold line-through ${isPopular || isVIP ? 'text-stone-400' : 'text-stone-400'}`}>
                      ...
                      <div className={`p-3.5 rounded-2xl mb-6 text-xs font-semibold space-y-2 border ${isPopular
                        ...
                        <div className="flex items-center justify-between">
                          ...
                          <span className="font-extrabold text-amber-300">{plan.contactUnlocks}</span>
                        </div>
                        <div className="flex items-center justify-between">
                          <span className="opacity-90">Search Ranking:</span>
                          <span className="font-extrabold text-amber-300">{plan.profileBoost}</span>
                        </div>
                      </div>
                      ...
                      <ul className="space-y-3 text-xs">
                        {plan.features.map((f, idx) => (
                          <li key={idx} className="flex items-start gap-2.5">
                            ...
                            <span className={isPopular || isVIP ? 'text-stone-100 font-medium' : 'text-stone-700 font-medium'}>{f}</span>
                          </li>
                        ))}
                      </ul>
                    </div>
                    ...
                    <div className="mt-8 pt-4">
                      <Button
                        ...
                        className={`w-full font-extrabold shadow-xl h-12 text-xs uppercase tracking-wider transition-all duration-300 ${isPopular
                          ...
                          : isVIP
                            ? 'bg-gradient-to-r from-[#8B1E3F] to-[#C44569] text-white hover:brightness-110 shadow-rose-900/30'
